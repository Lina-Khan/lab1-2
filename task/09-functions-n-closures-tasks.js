...
function getPolynom() {
    return x=>{
        if(arguments.length === 0)
            return null;
        return Array.from(arguments).reverse().reduce((rez,val,i,arr) => {
            return rez + val*Math.pow(x,i);
        },0);
    }
}
...
function parseArg(val)
{
    ...
    else if(val instanceof Array)
    {
        let rez = [];
        for(let i in val)
            rez.push(parseArg(val[i]));
        if(rez.length === 1)
            return rez[0].toString();
        else
            return "["+rez.join(',')+"]";
    }
    else
